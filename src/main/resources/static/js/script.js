...
// EVENT LISTENERS
    ...
    task.forEach(task => {
        ...
        taskBody.addEventListener('click',  () => {
            const isComplete = taskBody.classList.contains('complete');
            const isNotComplete = taskBody.classList.contains('not-complete')
            console.log(isComplete)
            console.log(typeof isComplete)
            console.log(taskBody.classList + 'before conditional')
            if (isComplete) {
                taskBody.classList.remove('not-complete')
                console.log(taskBody.classList + ' isComplete')
            }
            if (isNotComplete) {
                taskBody.classList.remove('complete');
                console.log(`${taskBody.classList} from not complete task`)
            }
            const numOfTasks = document.querySelectorAll('.task').length;
            console.log(numOfTasks + ' before monsterDamage()')
            const monsterDamage = (numOfTasks) => {
                const healthBar = document.querySelector('.health-bar');
                console.log(`healthBar: ${healthBar}`)
                const currentHealth = parseFloat(healthBar.style.width);
                console.log(`currentHealth: ${currentHealth}`)
                const damageOutput = (100 / numOfTasks).toFixed(2);// import * as keys from './keys.js';
(() => {
// DOM ELEMENTS
    const battleIcon = document.querySelector('.battle-icon')
    const battleIconBanner = document.querySelector('.battle-icon-banner')
